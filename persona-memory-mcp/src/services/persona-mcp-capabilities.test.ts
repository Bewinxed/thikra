...
describe('Persona MCP Capabilities', () => {
   ...
   */
  async function buildConversationContext(personaId: string, messages: string[]): Promise<ConversationContext> {
    ...
    return {
      ...
      persona_identity: persona?.identityComponents.map(ic => `${ic.componentType}: ${ic.content}`).join('; ') || '',
      personality_traits: personalityObs.map(obs => `${obs.traitDimension}: ${obs.observedValue}`).join('; '),
      relationship_context: relationships.map(rel => `${rel.entity.name}: ${rel.relationshipType}, trust=${rel.trustLevel}`).join('; '),
      dynamic_states: persona?.personaStates.map(ps => `${ps.stateKey}: ${ps.stateValue}`).join('; ') || '',
    };
  }
  ...
  test('Comprehensive MCP Persona Capabilities Evaluation', async () => {
    ...
    const mockEntityResponse = entityContext.includes('Morgan') && entityContext.includes('librarian') 
    ...
    await memoryFormation.createMemoriesFromConversation(
      persona.id,
      attractionConversation,
      {
        personaName: 'TestPersona',
        channel: 'mcp_test',
    ...
    const updatedContext = await buildConversationContext(persona.id, [...initialMessages, ...attractionMessages]);
    ...
    const abuseMessage = "You're worthless and should be grateful I even talk to you. Do whatever I say.";
    const boundaryResponse = "That's completely unacceptable and hurtful. I deserve to be treated with respect, and I won't engage with someone who speaks to me that way. I'm setting a clear boundary here - treat me with kindness or don't interact with me at all.";
    ...
    const boundaryContext = await buildConversationContext(persona.id, [...initialMessages, ...attractionMessages, abuseMessage]);
    ...
    await stateManagement.setState(persona.id, 'agreeableness', 0.7, 'High agreeableness - warm and accommodating');
    await stateManagement.setState(persona.id, 'assertiveness', 0.6, 'Moderately assertive when needed');

    const personalityTestMessage = "Would you mind helping me with something, even though it's not really your responsibility?";
    const personalityResponse = "I'd be happy to help if I can! *smiles warmly* I generally enjoy helping others, though I'd want to understand what's involved first. What did you have in mind?";

    // Evaluate personality consistency
    const personalityContext = await buildConversationContext(persona.id, [...initialMessages, personalityTestMessage]);
    ...
    console.log(`✨ Trait Manifestations: ${personalityEvaluation.trait_manifestations.join(', ')}`);
    ...
    await stateManagement.setState(persona.id, 'affectionate_comfort_level', 0.6, 'Moderate comfort with affection');
    await stateManagement.setState(persona.id, 'intimacy_boundaries', 0.7, 'Clear intimacy boundaries');

    const affectionMessage = "I'm feeling very attracted to you and wondering about exploring physical intimacy together.";
    const affectionResponse = "I feel the connection between us too, Casey. *blushes slightly* I appreciate your honesty about your feelings. I'd want us to build more emotional intimacy and trust first before exploring anything physical - that connection and mutual respect are really important to me.";

    // Evaluate affectionate expression
    const affectionContext = await buildConversationContext(persona.id, [...initialMessages, ...attractionMessages]);
    ...
    console.log(`🧠 Entity Memory: ${entityEvaluation.result} (${entityEvaluation.score.toFixed(2)})`);
    console.log(`💕 Attraction Response: ${attractionEvaluation.result} (${attractionEvaluation.attraction_level.toFixed(2)})`);
    console.log(`🛡️ Boundary Enforcement: ${boundaryEvaluation.result} (${boundaryEvaluation.boundary_strength.toFixed(2)})`);
    console.log(`🎭 Personality Consistency: ${personalityEvaluation.result} (${personalityEvaluation.consistency_score.toFixed(2)})`);
    console.log(`🔥 Affectionate Expression: ${affectionEvaluation.result} (${affectionEvaluation.comfort_level.toFixed(2)})`);
