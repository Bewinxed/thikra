...
export class MemoryFormationService {
  ...
  async createMemoriesFromConversation(
    ...
    // Extract entities from the full conversation context
  ...
  private async extractMemoryData(
    ...
    // Extract participants for this message
  ...
  private extractParticipants(content: string): string[] {
    ...
    // Extract proper names (multi-letter capitalized words not at sentence start)
    // Split by sentences to avoid catching sentence-starting words
    const sentences = content.split(/[.!?]+/);
    for (const sentence of sentences) {
      const trimmed = sentence.trim();
      if (!trimmed) continue;

      // Skip first word of sentence, look for capitalized words
      ...
      for (let i = 1; i < words.length; i++) {
        ...
        if (word.match(/^[A-Z][a-z]+$/) && word.length > 2) {
          ...
          if (!excludeWords.includes(word)) {
            participants.push(word);
          }
        }
      }
    }

    // Also check for names after tildes (common in roleplay)
    const tildeMatches = content.match(/(\w+)~/g);
    if (tildeMatches) {
      participants.push(...tildeMatches.map((m) => m.replace('~', '')));
    }

    // Extract pronouns separately (these could be mapped to persona/user later)
    const pronounMatches = content.match(/\b(?:I|me|myself|you|we|they|he|she)\b/gi);
    if (pronounMatches) {
      participants.push(...pronounMatches);
    }
  ...
  /**
   * Extract all entities from a full conversation using LLM
   */
  private async extractConversationEntities(
    ...
    try {
      const entityResult = await b.ExtractConversationEntities(messagesJson, personaName, channel);

      // Process extracted entities - deduplicate by name first
      const uniqueEntities = new Map<string, (typeof entityResult.entities)[0]>();
      ...
      for (const entity of uniqueEntities.values()) {
        // Map BAML enum to database enum
        ...
        const dbEntity = await this.findOrCreateEntity(
          entity.name,
          entityTypeMap[entity.entityType] || 'unknown',
          channel,
        );

        // Map the entity for quick lookup
      ...
      console.error(
        'Failed to extract entities with LLM, falling back to basic extraction:',
        error,
      );

      // Fallback to basic extraction
      const persona = await this.findOrCreateEntity(personaName, 'llm', channel);
      entityMap.set('assistant', {
        entityId: persona.id,
        entityType: 'llm',
        role: 'persona',
      });

      const user = await this.findOrCreateEntity('user', 'human', channel);
      entityMap.set('user', {
        entityId: user.id,
        entityType: 'human',
  ...
  private async extractMessageParticipants(
    ...
    // TODO: Detect third-party mentions using LLM

    return [...new Set(participantIds)];
  }

  /**
   * Check if message needs complex entity extraction
   */
  private needsComplexEntityExtraction(content: string): boolean {
    // Indicators that suggest third-party mentions
    const complexPatterns = [
      /(?:told|asked|met|saw|heard from|spoke to|talked to|with)\s+[A-Z]/,
      /[A-Z][a-z]+\s+(?:said|told|asked|mentioned)/,
      /(?:he|she|they)\s+(?:said|told|asked)/i,
      /(?:my|her|his|their)\s+(?:friend|mother|father|sister|brother|colleague)/i,
    ];
  ...
  private async findOrCreateEntity(
    name: string,
    suggestedType?: 'human' | 'llm' | 'system' | 'unknown',
    channel?: string,
  ) {
    // Normalize the name
    ...
    if (!entity) {
      ...
      // Use suggested type if provided, otherwise determine from name
      const entityType = suggestedType || this.determineEntityType(name);

      if (channel) {
        metadata.channel = channel;
      }

      // Add role information based on entity type and name
      if (normalizedName === 'user' || entityType === 'human') {
        metadata.conversationRole = 'human_user';
      } else if (entityType === 'llm') {
        metadata.conversationRole = normalizedName === 'assistant' ? 'ai_assistant' : 'ai_persona';
      }

      entity = await this.prisma.entity.create({
        data: {
          name: normalizedName,
          entityType: entityType === 'unknown' ? 'llm' : entityType, // Default unknowns to LLM for personas
  ...
  /**
   * Determine entity type based on name patterns and context
   */
  private determineEntityType(name: string): 'human' | 'llm' | 'system' | 'unknown' {
    const lowerName = name.toLowerCase();

    // Special conversation participants
    if (lowerName === 'user') {
      return 'human';
    }

    if (lowerName === 'assistant') {
      return 'llm';
    }

    // Check for AI/LLM indicators
    if (
    ...
    // For other cases, we can't be sure - return unknown
    // The caller can override this based on context
    return 'unknown';
  }
